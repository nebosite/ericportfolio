...
describe('public feedback API', () => {
  ...
  it('validates entity and text length', async () => {
    const { app } = fresh();
    expect((await request(app).post('/api/feedback').send({ entity: 'BAD!', text: 'hi' })).status).toBe(400);
  ...
  it('does not offer implemented requests for voting', async () => {
    const { db, app } = fresh();
    const keep = await request(app).post('/api/feedback').send({ entity: 'snake', text: 'still open' });
    const done = await request(app).post('/api/feedback').send({ entity: 'snake', text: 'already done' });
...
describe('admin auth', () => {
  ...
  it('closes the admin API entirely when no token is configured', async () => {
    ...
    expect((await request(app).get('/api/admin/feedback').set('Authorization', 'Bearer x')).status).toBe(
      401,
    );
  });
});
...
describe('admin list', () => {
  ...
  it('flags items created since the last check and advances the marker', async () => {
    ...
    db.prepare("INSERT INTO feedback (entity, text, created_at) VALUES ('snake','old','2024-01-01 00:00:00')").run();
    db.prepare("UPDATE admin_meta SET last_seen = '2024-06-01 00:00:00' WHERE id = 1").run();
    db.prepare("INSERT INTO feedback (entity, text, created_at) VALUES ('snake','fresh','2024-12-01 00:00:00')").run();
...
describe('admin mutations', () => {
  it('changes status between the two allowed values', async () => {
    const { app } = fresh();
    const created = await request(app).post('/api/feedback').send({ entity: 'snake', text: 'do it' });
    const id = created.body.id;
    const ok = await auth(request(app).patch(`/api/admin/feedback/${id}`).send({ status: 'Implemented' }));
  ...
  it('edits notes and returns them on the admin list', async () => {
    const { app } = fresh();
    const created = await request(app).post('/api/feedback').send({ entity: 'snake', text: 'idea' });
    const id = created.body.id;
  ...
  it('can update status and notes together', async () => {
    const { app } = fresh();
    const created = await request(app).post('/api/feedback').send({ entity: 'snake', text: 'two at once' });
    const res = await auth(
      request(app)
  ...
  it('rejects an invalid status and an empty patch', async () => {
    ...
    expect(
      (await auth(request(app).patch(`/api/admin/feedback/${created.body.id}`).send({ status: 'Done' })))
        .status,
    ).toBe(400);
    expect(
  ...
  it('404s deleting or patching an unknown id', async () => {
    ...
    expect(
      (await auth(request(app).patch('/api/admin/feedback/999').send({ status: 'Implemented' }))).status,
    ).toBe(404);
  });
});
