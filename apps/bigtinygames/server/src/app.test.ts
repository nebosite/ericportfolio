...
describe("leaderboard", () => {
  ...
  it("rejects bad initials", async () => {
    const app = freshApp();
    expect(
      (await request(app).post("/api/leaderboard").send({ initials: "", score: 1 })).status,
    ).toBe(400);
    expect(
  ...
  it("rejects non-integer, negative, or absurd scores", async () => {
    const app = freshApp();
    expect(
      (await request(app).post("/api/leaderboard").send({ initials: "AB", score: -1 })).status,
    ).toBe(400);
    expect(
      (await request(app).post("/api/leaderboard").send({ initials: "AB", score: 1.5 })).status,
    ).toBe(400);
    expect(
      (await request(app).post("/api/leaderboard").send({ initials: "AB", score: 100_000_001 }))
        .status,
    ).toBe(400);
    expect(
  ...
  it("rejects an invalid game slug", async () => {
    ...
    expect(
      (
        await request(app)
          .post("/api/leaderboard")
          .send({ initials: "AB", score: 1, game: "Bad Game!" })
      ).status,
    ).toBe(400);
  });
});
