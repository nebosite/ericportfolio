...
describe("PaintApp", () => {
  ...
  it("has no always-on color strip; colors live behind the palette button", () => {
    ...
    expect(screen.queryByRole("button", { name: "Paint with #ff3b3b" })).not.toBeInTheDocument();
  });
  ...
  it("opens the color palette dialog and closes it with the ✕", () => {
    ...
    expect(screen.getByRole("button", { name: "Paint with #ff3b3b" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Animated color 1" })).toBeInTheDocument();
