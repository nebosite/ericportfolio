...
// however you like — the reference React renderer is at the bottom.
//
...
export function growMark(
  ...
  const grow = (x: number, y: number, ang: number, len: number, depth: number) => {
    ...
    segments.push({ x1: x, y1: y, x2, y2, w: Math.max(0.7, depth * 0.95) * strokeScale });
...
/* ----------------------------------------------------------------------------
 * Reference React renderer (delete if you render with raw SVG strings instead).
 *
 * import { growMark, Mark } from './growMark';
 *
 * function GrownMark({ seed, style, accent, garnish = true, strokeScale = 1, growthSeed = 1 }: {
 *   seed: string; style: GrowthStyle; accent: string;
 *   garnish?: boolean; strokeScale?: number; growthSeed?: number;
 * }) {
 *   const m: Mark = growMark(seed, style, strokeScale, growthSeed);
 *   const budFill = garnish ? accent : '#1d1b16';
 *   return (
 ...
 *               stroke="#1d1b16" strokeWidth={s.w} strokeLinecap={m.cap} />
 *       ))}
 *       {m.buds.map((b, i) => m.square
 *         ? <rect key={'b' + i} x={b.x - (4 * strokeScale + 1.2) / 2} y={b.y - (4 * strokeScale + 1.2) / 2}
 *                 width={4 * strokeScale + 1.2} height={4 * strokeScale + 1.2} fill={budFill} />
 *         : <circle key={'b' + i} cx={b.x} cy={b.y} r={2.4 * strokeScale + 0.6} fill={budFill} />
 *       )}
 *     </svg>
 *   );
 * }
