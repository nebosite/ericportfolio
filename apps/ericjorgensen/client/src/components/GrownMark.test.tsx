...
describe("GrownMark", () => {
  it("staggers branch growth with a per-generation delay (trunk first)", () => {
    const { container } = render(<GrownMark seed="x" style="wild" accent="#000" />);
    const lines = Array.from(container.querySelectorAll("line"));
    expect(lines.length).toBeGreaterThan(0);

    const delays = lines.map((l) => (l as SVGLineElement).style.getPropertyValue("--gm-delay"));
  ...
  it("gives each line a dash length so it can draw in", () => {
    const { container } = render(<GrownMark seed="y" style="sapling" accent="#000" />);
